...
if __name__ == "__main__":
    # Collect telemetry data based on the current platform
    if platform.system() == 'Darwin':
        telemetry_data = collect_telemetry_macos()
    elif platform.system() == 'Windows':
        telemetry_data = collect_telemetry_windows()
    else:
        raise NotImplementedError("Unsupported OS")
    
    # Analyze telemetry data
    cpu_range, memory_range, disk_range = analyze_telemetry(telemetry_data)
    
    # Estimate average power consumption
    avg_cpu_power, avg_memory_power, avg_disk_power = estimate_power(telemetry_data)
    ...
    # Display suggestions in a GUI window
